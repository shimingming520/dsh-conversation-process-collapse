	...
	factory: (require) => {
		...
		*/
		...
		function imageCardOf(view) {
			...
			const record = view;
			if (record.card !== "image") return void 0;
		...
		*  card, so no tool-specific card union needs importing. */
		...
		function nodeRow(key, surface, turn, byTurn) {
			...
			const images = plan !== void 0 && (plan.resultKey === key || plan.resultKey === void 0 && plan.imageKey === key) ? plan.images : void 0;
			return images !== void 0 && images.length > 0 ? {
				kind: "node",
				key,
				surface,
				images
